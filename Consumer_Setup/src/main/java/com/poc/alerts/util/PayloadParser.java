...
public class PayloadParser {
	...
	public static Map<String,String> extractFieldsBasedOnAlertType(
		...
		templateLog.info("alert Type:{}",alertType);
	    ...
	    JsonNode custom = root
	            .path("payload")
	            .path("customFieldDetails");
	    
	    JsonNode customCommon = root
	            .path("payload")
	            .path("eventCommon")
	            .path("customCommon");
	    
	    templateLog.info("customCommon Data: {}", customCommon);
	    
	    templateLog.info("customFieldDetails Data:{}",custom);
	    
	    JsonNode eventCommon = root
	            .path("payload")
	            .path("eventCommon");
	    
	    templateLog.info("eventCommon Data:{}",eventCommon);
	    ...
	    switch(alertType) {
	        ...
	        case "LOAN_DISBURSEMENT_SUCCESSFUL":
	            ...
	            if(loanAccount != null && loanAccount.length() >= 4){
	                loanLast4 = loanAccount.substring(loanAccount.length() - 4);
	            }
	        ...
	        case "FUND_TRANSFER_SUCCESSFUL":
	            ...
	            if(senderAccount != null && senderAccount.length() >= 4){
	                accountLast4 =
	                    senderAccount.substring(senderAccount.length()-4);
	            }
	            ...
	            if(receiverAccount != null && receiverAccount.length() >= 4){
	                beneficiaryMasked =
	                    "****" + receiverAccount.substring(receiverAccount.length()-4);
	            }
    ...
    @SuppressWarnings("deprecation")
	public static ConsumerEntryAudit parse(String topic, String json) {

    	ConsumerEntryAudit audit = new ConsumerEntryAudit();
        ...
        try {
            ...
            audit.setApplication(
                    payload.path("eventCommon").path("application").asText()
            );

            JsonNode custom =
                    payload.path("customFieldDetails");

            audit.setMessageType(custom.path("MessageType").asText());
            audit.setAlertType(custom.path("alertType").asText());
            audit.setReleaseVersion(custom.path("Release").asText());
